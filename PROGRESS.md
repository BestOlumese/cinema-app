...
- **Deliberately not done yet**: formal checked-in Playwright test files. `phases/phase-1.md`'s own DoD bundles "Playwright coverage on signup + invitation flow" as one line item, and the invitation flow (Task 4) doesn't exist yet — writing Playwright infra now for signup alone risks being reworked once invitation exists. Flagged to the human as an open sequencing question rather than silently deferred or silently done early.
